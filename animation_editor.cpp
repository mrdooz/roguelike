...
struct Bolt
{
  Bolt(const Pos& s, const Pos& e, int steps) : start(s), end(e), steps(steps) {}
  Pos start, end;
  int steps;
};
...
void CanvasWindow::LightningBolt()
{
  ...
  deque<Bolt> bolts;
  bolts.push_back(Bolt(Pos(GaussianRand(w/2, w/4), 0), Pos(GaussianRand(w/2, w/4), h-1), 3));

  memset(_frameDoubleBuffer.data(), 0, w*h*4);
  int totalBolts = 1;
  ...
  while (!bolts.empty())
  {
    HsvColor c(_editor->_primaryColor);
    c.s /= colorScale;
    u32 color(ColorToU32(c));

    Bolt cur = bolts.front();
    bolts.pop_front();

    vector<Pos> points;
    points.push_back(cur.start);
    points.push_back(cur.end);

    for (int i = 0; i < cur.steps; ++i)
    {
      // choose random endpoint
      int idx = 1 + rand() % (points.size() - 1);
      const Pos& s = points[idx-1];
      const Pos& e = points[idx];
      int x = Clamp<int>(s.x + GaussianRand(e.x-s.x, w), 0, w-1);
      int y = Clamp<int>(s.y + GaussianRand(0.5f * (e.y-s.y), 0.5f * (e.y-s.y)), 0, h-1);

      Pos p(x,y);
      if (totalBolts < 3)
      {
        ++totalBolts;
        // create a new bolt at the current location
        int xx = Clamp<int>(p.x + GaussianRand(w/2, w/4), 0, w-1);
        bolts.push_back(Bolt(p, Pos(xx, h-1), cur.steps-1));
      }

      points.insert(points.begin() + idx, p);
    }
    ...
    {
      Pos e(points[i]);
      Line(s.x, s.y, e.x, e.y, &line);

      for (const Pos& p : line)
      {
        u32* dst = (u32*)&_frameDoubleBuffer[(p.y*w+p.x)*4];
        *dst += color;
      }
      s = e;
    }
  ...
  , _animationManager(animationManager)
{
  ...
  _windowManager.AddWindow(_imguiWindow);
...
bool AnimationEditor::OnKeyReleased(const Event& event)
{
  ...
  else
  {
    ...
    else if (code == Keyboard::L)
    {
      _historyWindow->AddHistory("LightningBolt");
      _canvasWindow->LightningBolt();
    }
